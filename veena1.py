...
def input_page(title, fields):
    ...
    if st.button("Predict"):
        try:
            # Convert all inputs to float and calculate sum
            values = [float(v) for v in user_inputs.values() if v.strip() != ""]
            prediction = sum(values)  # dummy calculation
